/**
 * Created by Dima on 18.10.2020.
 */

let graph_matrix_one_empty_line =
    [
        {
        ...
        //конец серой ветки 197
        {
            name: "Поселок Северный",
            weight: 0,
            cost: 0,
        },
        {
        ...
        //конец серо-голубой ветки 236
        //продолжение бирюзовой ветки 237
        {
            name: "Можайская",
            weight: 0,
            cost: 0,
...
class Graph {
    ...
    add_line_in_matrix(crossing_station, index) {
        let line = JSON.parse(JSON.stringify(this.matrix_line));
        for (let i = 0; i < crossing_station.length; i++)
            line[this.find_in_matrix_line(crossing_station[i].name, index)] = crossing_station[i];
        this.metro_station_graph.push(line);
    }

    find_in_matrix_line(name, current_index) {
        for (let i = 0; i < this.matrix_line.length; i++) {
            if (i !== current_index) {
                if (this.matrix_line[i].name === name)
                    return i;
            }
        }
    }
